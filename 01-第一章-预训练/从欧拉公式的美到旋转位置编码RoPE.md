> 额外参考阅读：https://zhuanlan.zhihu.com/p/8306958113
...
> fish: $x_{m}$是指第m个token，$f_{q}(x_{m},m)$是指$x_{m}$经过线性层并赋予$m$位置后得到的最终$query$值(第$m$个token)
>
> 上述表达式即携带位置信息的Query的第$m$个token和携带位置信息的key的第$n$个token的点积结果（attention-map的点积值）可以基于相对位置$m-n$来计算
...
> fish: 当前只考虑dim=2的情况。其中$q_{m}^{(1)}$是指第m个token的第1个维度的数值，$q_{m}^{(2)}$是指第m个token的第2个维度的数值
